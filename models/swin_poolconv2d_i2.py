...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = nn.Conv2d(in_features, hidden_features, 1, groups = in_features)
        weight = self.fc1.weight.data.numpy()
        print(weight.shape)
        #self.fc1 = nn.Conv2d(in_features, hidden_features, 1)
        self.act = act_layer()
        #self.fc2 = nn.Linear(hidden_features, out_features)
        self.fc2 = nn.Conv2d(hidden_features, out_features, 1, groups = out_features)
        weight = self.fc2.weight.data.numpy()
        print(weight.shape)
    ...
    def forward(self, x):
        ...
        #x = x.view(B , C, H, W) #probably reshape is not correct 

        #amount of parameter size depends on the parameter C and kernel size K
        #without specification, K is equal 3 
        #memory size is relative to C? C^2 * K 

        # the current memory = C^2 * K^2 
        # my current implementation = H^2 * K^2
